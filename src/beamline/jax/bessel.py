...
Here we take the example implementation from that PR and adapt it
...
def _jv_forward_recurrence(v: Array, x: Array) -> Array:
    ...
    def body(state: _FwdState) -> _FwdState:
        jnm1, jn, n = state
        term = (2 * n) / x * jn
        jnplus = term - jnm1
        return jn, jnplus, n + 1

    _, jn, _ = jax.lax.while_loop(cond, body, (_j0(x), _j1(x), jnp.ones_like(v)))
    return jn
...
def _jv_backward_recurrence(v: Array, x: Array) -> Array:
    ...
    def body(state: _BwdState) -> _BwdState:
        jn, jnp1, n, out = state
        out = jnp.where(n == v, jn, out)
        term = (2 * n) / x * jn
        jnminus = term - jnp1
        return jnminus, jn, n - 1, out
...
def jv(v: Array, x: Array) -> Array:
    ...
    This does not yet handle orders other than integers.

    Args:
        v: Order of the Bessel function (integer)
        x: Real valued input
    ...
    # reflect v if negative
    vfact = jnp.where(v < 0, jnp.pow(-1.0, v), 1.0)
    v = jnp.abs(v)
