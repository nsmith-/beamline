...
def main():
    ...
    # For context: one disk's loss alone, and a single solid disk of combined length.
    out1, _ = jax.jit(jax.vmap(disk1.apply))(beam, keys)
    dE_one_disk = np.asarray(beam.kin.t.ct - out1.kin.t.ct)
    solid = CylindricalAbsorber(material=mat, radius=RADIUS * u.mm,
                                length=(DISK1_LEN + DISK2_LEN) * u.mm)
    outS, _ = jax.jit(jax.vmap(solid.apply))(beam, keys)
    dE_solid = np.asarray(beam.kin.t.ct - outS.kin.t.ct)
    ...
    print(f"one solid {DISK1_LEN+DISK2_LEN:.0f}mm  : mode {mode(dE_solid):.3f}  median {np.median(dE_solid):.3f} MeV")
